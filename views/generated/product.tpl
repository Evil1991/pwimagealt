...
{if $errors|@count == 0}
	...
	<div class="product_info">
		...
		{/if}
		<!-- left infos-->  
		<div class="pb-left-column {if $wd.style_replaceblock=="custom_html"}span4 {elseif $wd.style_replaceblock=="special_block"}span4 {elseif  $wd.style_replaceblock=="bestsell_block"}span4 {elseif  $wd.style_replaceblock=="no"}span4 {else}pb-left-column span4{/if}">
			<!-- product img-->        
			<div id="image-block" class="clearfix">
            
{if $wd.style_disnew_badge=='yes'}
{if $product->new}
<span class="product-new">
<span class="style-01">{l s='exclusive'}</span>
</span>
{/if}
{/if}

{if $wd.style_dissale_badge=='yes'}
{if $product->on_sale}
<span class="product-sale">
<span class="style-01">{l s='Sale!'}</span>
</span>
{elseif $product->specificPrice && $product->specificPrice.reduction && $productPriceWithoutReduction > $productPrice}
<span class="product-sale">
<span class="style-01">{l s='Reduced price!'}</span>
</span>
{/if}
{/if}
				{if $have_image}
					<span id="view_full_size">
						{if $jqZoomEnabled && $have_image && !$content_only}
							<a class="jqzoom" rel="gal1" href="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'thickbox_default')|escape:'html':'UTF-8'}" itemprop="url">
								<img alt="{$product->name}" title="{$product->name}" itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html':'UTF-8'}" alt="{if !empty($cover.legend)}{$cover.legend|escape:'html':'UTF-8'}{else}{$product->name|escape:'html':'UTF-8'}{/if}"/>
							</a>
						{else}
							<img alt="{$product->name}" title="{$product->name}" id="bigpic" itemprop="image" src="{$link->getImageLink($product->link_rewrite, $cover.id_image, 'large_default')|escape:'html':'UTF-8'}" width="{$largeSize.width}" height="{$largeSize.height}"/>
							{if !$content_only}
								<span class="span_link no-print" title="{l s='View larger'}"><i class="fa fa-search-plus"></i></span>
							{/if}
						{/if}
					</span>
				{else}
					<span id="view_full_size">
						<img alt="{$product->name}" title="{$product->name}" itemprop="image" src="{$img_prod_dir}{$lang_iso}-default-large_default.jpg" id="bigpic" width="{$largeSize.width}" height="{$largeSize.height}"/>
						{if !$content_only}
							...
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
									<a 
										...
										<img alt="{$product->name}" title="{$product->name}" class="img-responsive" id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'cart_default')|escape:'html':'UTF-8'}" height="{$cartSize.height}" width="{$cartSize.width}" itemprop="image" />
									</a>
								</li>
							{/foreach}
						{/if}
...
<div class="{if $wd.style_replaceblock=="custom_html"}span9 {elseif $wd.style_replaceblock=="special_block"}span9 {elseif  $wd.style_replaceblock=="bestsell_block"}span9{elseif  $wd.style_replaceblock=="no"}span12{else}span12{/if}">
{if isset($HOOK_EXTRA_RIGHT) && $HOOK_EXTRA_RIGHT}{$HOOK_EXTRA_RIGHT|replace:"h2":"div"}{/if}
...
{if $product->condition}
            ...
            <a href="{$link->getManufacturerLink($product_manufacturer->id)}" class="editable">{$product_manufacturer->name|escape:'html':'UTF-8'}</a>
            </p>
            {/if}
{if ($display_qties == 1 && !$PS_CATALOG_MODE && $PS_STOCK_MANAGEMENT && $product->available_for_order)}
			...
			<div id="oosHook"{if $product->quantity > 0} style="display: none;"{/if}>
				{$HOOK_PRODUCT_OOS|replace:"h3":"div"|replace:"h2":"div"}
			</div>
														...
														{foreach from=$group.attributes key=id_attribute item=group_attribute}
															...
															<li{if $group.default == $id_attribute} class="selected"{/if}>
																<a href="{$link->getProductLink($product)|escape:'html':'UTF-8'}" id="color_{$id_attribute|intval}" name="{$colors.$id_attribute.name|escape:'html':'UTF-8'}" class="color_pick{if ($group.default == $id_attribute)} selected{/if}"{if !$img_color_exists && isset($colors.$id_attribute.value) && $colors.$id_attribute.value} style="background:{$colors.$id_attribute.value|escape:'html':'UTF-8'};"{/if} title="{$colors.$id_attribute.name|escape:'html':'UTF-8'}">
																	{if $img_color_exists}
																		<img alt="{$product->name}" title="{$product->name}" src="{$img_col_dir}{$id_attribute|intval}.jpg" width="20" height="20" />
																	{/if}
																</a>
															</li>
...
<div class="short_description_pack">
			...
			{if !$content_only}
				...
				<ul id="usefull_link_block" class="clearfix no-print">
					{if $HOOK_EXTRA_LEFT}{$HOOK_EXTRA_LEFT|replace:"h3":"div"|replace:"h2":"div"}{/if}
					<li class="print">
			...
			{/if}
{if $wd.style_sharenable=='yes'}
{$wd.style_sharecode|html_entity_decode}
{/if}
    </div>        

<!-- pb-right-column-->
{if $wd.style_replaceblock!='no'}
<div class="pb-right-column span3">
{if $wd.style_replaceblock=='custom_html'}
{$wd.style_product_page_markup|html_entity_decode}
{else}
{$PRO_CUSTOM_RIGHT_COLUMN}
{/if}
</div>
{/if}
<!-- end pb-right-column-->
...
{$wd.style_probtmcus|html_entity_decode|replace:"h2":"div"}
	{if !$content_only}
{if (isset($quantity_discounts) && count($quantity_discounts) > 0)}
			<!-- quantity discount -->
			<section class="page-product-box">
				<div class="page-product-heading">{l s='Volume discounts'}</div>
...
<div id="more_info_block" class="clear cre-animate" data-animation="slide-up-from-bottom" data-delay="250" data-easing="easeInOutBack" data-offset="80%" data-speed="1000">
    <ul id="more_info_tabs" class="idTabs idTabsShort clearfix">
        ...
        {$HOOK_PRODUCT_TAB|replace:"h3":"div"|replace:"h2":"div"}
        {if $wd.style_protabs=='yes'} 
<li>
    <a href="#idtabcustom">
        {$wd.style_protabtitle}
    </a>
</li>
{/if}
    </ul>
                        ...
                        {if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
                            ...
                            <li class="ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
                                ...
                                <div class="product_desc">
                                    <a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" class="product_image"><img alt="{$product->name}" title="{$product->name}" src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'medium_default')|escape:'html'}" width="{$mediumSize.width}" height="{$mediumSize.height}" /></a>
        ...
        <div id="idTab10" class="bullet customization_block">
            <form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm" class="clearfix">
                            ...
                            {if $field.type == 0}
                                <li class="customizationUploadLine{if $field.required} required{/if}">{assign var='key' value='pictures_'|cat:$product->id|cat:'_'|cat:$field.id_customization_field}
                                    ...
                                    <div class="customizationUploadBrowse">
                                        <img alt="{$product->name}" title="{$product->name}" src="{$pic_dir}{$pictures.$key}_small" />
                                        <a href="{$link->getProductDeletePictureLink($product, $field.id_customization_field)|escape:'html'}" title="{l s='Delete'}" >
                                            <img alt="{$product->name}" title="{$product->name}" src="{$img_dir}icon/delete.gif" alt="{l s='Delete'}" class="customization_delete_icon" width="11" height="13" />
                                        </a>
                                    </div>
                                    {/if}
                                    <div class="customizationUploadBrowse">
                ...
                <p id="customizedDatas">
                    ...
                    <span id="ajax-loader" style="display:none"><img alt="{$product->name}" title="{$product->name}" src="{$img_ps_dir}loader.gif" /></span>
                </p>
            </form>
    ...
    {if isset($HOOK_PRODUCT_TAB_CONTENT) && $HOOK_PRODUCT_TAB_CONTENT}{$HOOK_PRODUCT_TAB_CONTENT|replace:"h3":"div"|replace:"h2":"div"}{/if}
    {if $wd.style_protabs=='yes'} 
<div id="idtabcustom" class="rte">
    {$wd.style_protabcontent}
</div>
{/if}
    </div>
</div>
...
{if isset($packItems) && $packItems|@count > 0}
    <div id="blockpack">
        <div>{l s='Pack content'}</div>
        {include file="$tpl_dir./product-list.tpl" products=$packItems}
    </div>
{/if}
 
{if isset($HOOK_PRODUCT_FOOTER) && $HOOK_PRODUCT_FOOTER}{$HOOK_PRODUCT_FOOTER|replace:"h3":"div"|replace:"h2":"div"}{/if}
	{/if}
{strip}
